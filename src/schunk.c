...
* @Last Modified time: 2015-12-03 11:03:10
*/
...
static int
slip_callNodeList(slip_Core* vm, slip_Node* node) {
	...
	if (v.t == slipV_light_cmacro_t) {
		...
		slipV_pushValue(vm, v);
		int ret_size = slipV_call(vm, 0);
		return ret_size;
	}

	if (v.t == slipV_chunk_t) {
...
slipC_callNode(slip_Core* vm, slip_Node* node) {
	switch (node->b.stype) {
		...
		case slipL_list_t: return slip_callNodeList(vm, node->l.child); 
	}
	printf("该Chunk无法调用\n");
	return -1;
}
